    ...
    <>
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-6 pt-24 pb-20 text-center">
          <div className="inline-flex items-center gap-2 bg-card border border-slate-700 rounded-full px-4 py-1.5 text-sm text-slate-400 mb-8">
      ...
      <section className="border-y border-slate-800 py-8">
        <div className="max-w-7xl mx-auto px-6">
          <p className="text-center text-sm text-slate-500 mb-6 uppercase tracking-widest">Trusted by teams at</p>
          <div className="flex flex-wrap items-center justify-center gap-8 md:gap-14">
            {TRUSTED_COMPANIES.map((c) => (
              <div key={c.name} className="flex items-center gap-2 text-slate-500 hover:text-slate-300 transition-colors">
                <span className="w-8 h-8 rounded-lg bg-slate-800 flex items-center justify-center text-sm font-bold">{c.logo}</span>
      ...
      <section className="max-w-7xl mx-auto px-6 pb-20">
        <div className="flex items-end justify-between mb-8">
          <div>
            <h2 className="text-3xl font-bold text-white">Featured Remote Jobs</h2>
            <p className="text-slate-400 mt-1">Hand-picked opportunities from top companies</p>
          </div>
        ...
        <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-4">
          {FEATURED_JOBS.map((job) => (
            <Link key={job.title + job.company} href="/jobs" className="card hover:border-primary/50 hover:bg-slate-800/60 transition-all group cursor-pointer">
              <div className="flex items-start justify-between mb-3">
                ...
                <span className={`badge ${job.type === 'Full-time' ? 'bg-green-900/40 text-green-400' : 'bg-orange-900/40 text-orange-400'}`}>
                  {job.type}
                </span>
              </div>
              <h3 className="font-semibold text-white group-hover:text-primary transition-colors mb-1">{job.title}</h3>
              <p className="text-sm text-slate-400 mb-1">{job.company}</p>
              <p className="text-xs text-slate-500 mb-4">{job.location}</p>
              <div className="flex flex-wrap gap-1.5 mb-4">
                {job.tags.map((tag) => (
                  <span key={tag} className="badge bg-slate-700/60 text-slate-400">{tag}</span>
                ))}
              </div>
              <p className="text-sm font-semibold text-accent">{job.salary}</p>
            </Link>
          ))}
        </div>
      ...
      {/* How it works */}
      <section className="bg-card border-y border-slate-800 py-20">
        <div className="max-w-7xl mx-auto px-6 text-center">
          <h2 className="text-3xl font-bold text-white mb-3">How JobConnect AI Works</h2>
          <p className="text-slate-400 mb-14 max-w-xl mx-auto">Three steps to your next remote role</p>
      ...
      <section className="max-w-7xl mx-auto px-6 py-24 text-center">
        <h2 className="text-4xl font-bold text-white mb-4">Ready to land your dream remote job?</h2>
        <p className="text-slate-400 mb-8 text-lg">Join 2.4 million candidates who found their next role through JobConnect AI.</p>
