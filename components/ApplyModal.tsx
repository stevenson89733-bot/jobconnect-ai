...
export default function ApplyModal({
  ...
  if (applied) {
    return (
      <span className="inline-flex items-center gap-1.5 text-xs font-semibold text-green-400 bg-green-900/30 border border-green-800/50 rounded-lg px-3 py-1.5 whitespace-nowrap">
        ✓ Applied
      </span>
    )
  }
            ...
            <div className="flex items-start justify-between p-6 pb-4 border-b border-slate-800">
              <div>
                ...
                <p className="text-slate-400 text-sm mt-0.5">
                  {jobTitle} · <span className="text-slate-300">{company}</span>
                </p>
              </div>
              <button
            ...
            <form onSubmit={handleApply} className="p-6 space-y-4">
              <div>
                <label className="block text-sm text-slate-400 mb-1.5">
                  Message to the hiring team <span className="text-slate-600">(optional)</span>
                </label>
                <textarea
                  value={message}
