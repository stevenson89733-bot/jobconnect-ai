          ...
          <div className="flex items-center justify-between mb-1">
            ...
            {totalActivity > 0 && (
              <div className="flex items-center gap-3 text-xs text-slate-600 dark:text-slate-400">
                <span className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-primary" /> {t('applications')}</span>
                <span className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-accent" /> {t('savedJobs')}</span>
              </div>
            )}
          </div>
          <p className="text-xs text-slate-600 dark:text-slate-400 mb-5">{t('last8Weeks')}</p>

          {totalActivity === 0 ? (
            <div className="text-center py-10">
              <div className="text-3xl mb-2">📈</div>
              <p className="text-sm text-slate-600 dark:text-slate-400">
                {t('weeklyActivityEmpty')}
              </p>
            </div>
          ) : (
                ...
                <div key={i} className="flex-1 flex flex-col items-center gap-1.5 h-full justify-end">
                  <div className="flex items-end gap-0.5 h-full w-full justify-center">
                    <motion.div
                      ...
                      title={`${w.applications} ${t('applications')}`}
                    />
                    <motion.div
                      initial={{ height: 0 }}
